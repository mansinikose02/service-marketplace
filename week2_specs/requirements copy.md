...
8. WHEN a profile is successfully saved, THE Profile_Service SHALL return the full ProviderProfile document, including the computed pitchComplete value.
...
2. THE Profile_Service SHALL apply the same field validation rules on update as on creation; IF any supplied field fails its validation rule, THEN THE Profile_Service SHALL return a 400 status with a descriptive error message.
...
5. WHEN a requirement is successfully updated, THE Requirement_Service SHALL return the full updated Requirement document.
...
4. WHEN a requirement is successfully closed, THE Requirement_Service SHALL return the full updated Requirement document with status "closed".
5. WHILE a Requirement's status is "closed", THE Requirement_Service SHALL reject any further update or close requests for that Requirement with a 409 status.
...
1. THE Auth_Guard SHALL permit only the authenticated Client_User who owns the Requirement to access its detail/edit page; WHEN any other user attempts to access the page, THE Auth_Guard SHALL redirect the user to the appropriate route.
